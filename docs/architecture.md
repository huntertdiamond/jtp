...
`jtp cd` prints a target path only. It does not `cd` itself.

Shell scripts generated by `jtp hook <shell>` or `jtp shell-init <shell>` wrap `jtp cd` and perform directory changes in the current shell (bash, zsh, fish). They also support auto-switching after successful `jtp add` in interactive TTY sessions.
