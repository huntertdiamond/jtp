...
func TestNormalizeCompletionArgs(t *testing.T) {
	t.Run("keeps trailing sentinel untouched", func(t *testing.T) {
		args := []string{"jtp", "remove", "target", "--", "--generate-shell-completion"}
		got := normalizeCompletionArgs(args)
		want := []string{"jtp", "remove", "target", "--", "--generate-shell-completion"}
	...
	t.Run("converts trailing sentinel in completion context", func(t *testing.T) {
		t.Setenv("COMP_LINE", "jtp remove target --")
		args := []string{"jtp", "remove", "target", "--", "--generate-shell-completion"}
		got := normalizeCompletionArgs(args)
		want := []string{"jtp", "remove", "target", "-", "--generate-shell-completion"}
	...
	t.Run("keeps completion flag before positional arguments", func(t *testing.T) {
		args := []string{"jtp", "remove", "--generate-shell-completion", "target"}
		got := normalizeCompletionArgs(args)
		want := []string{"jtp", "remove", "--generate-shell-completion", "target"}
...
func TestPatchCompletionScriptFishUsesDynamicHelper(t *testing.T) {
	original := `# jtp fish shell completion
complete -c jtp -a 'add'
`

	result := patchCompletionScript("fish", original)

	if !strings.Contains(result, "function __fish_jtp_dynamic_complete") {
		t.Fatalf("expected dynamic helper function in fish script, got:\n%s", result)
	}

	if !strings.Contains(result, "complete -c jtp -f -a '(__fish_jtp_dynamic_complete)'") {
	...
	if strings.Contains(result, "__fish_jtp_no_subcommand") {
...
func TestPatchCompletionScriptBashSanitizesDescriptions(t *testing.T) {
	...
	if !strings.Contains(result, "_jtp_sanitize_completion_list") {
		t.Fatalf("expected sanitize helper to be injected, got:\n%s", result)
	}

	if !strings.Contains(result, "opts=$(_jtp_sanitize_completion_list <<<\"${opts}\")") {
...
func TestPatchCompletionScriptZshInjectsCompletionEnv(t *testing.T) {
	const (
		...
		original    = "\n_jtp() {\n" + currentLine + "\n" + subCmdLine + "\n}\n"
		envMarker   = "env JTP_SHELL_COMPLETION=1"
	)
...
func generateCompletionScript(t *testing.T, shell string) string {
	...
	args := normalizeCompletionArgs([]string{"jtp", "completion", shell})
	...
	if runErr != nil {
		t.Fatalf("jtp completion %s failed: %v", shell, runErr)
	}
